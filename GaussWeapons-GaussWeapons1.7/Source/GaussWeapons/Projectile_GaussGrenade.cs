...
namespace GaussWeapons
{
    public class Projectile_GaussGrenade : Projectile
    {
        ...
        protected virtual void Explode()
        {
            ...
            GenExplosion.DoExplosion(Position, Map, def.projectile.explosionRadius, def.projectile.damageDef, launcher, DamageAmount, ArmorPenetration,  def.projectile.soundExplode, equipmentDef, null, null, def.projectile.postExplosionSpawnThingDef, def.projectile.postExplosionSpawnChance, 0, false, null, 0, 0);
            for (int i = 0; i < 4; i++)
            {
                ThrowSmokeBlue(Position.ToVector3Shifted() + Gen.RandomHorizontalVector(def.projectile.explosionRadius * 0.7f), Map, def.projectile.explosionRadius * 0.6f);
                ThrowMicroSparksBlue(Position.ToVector3Shifted() + Gen.RandomHorizontalVector(def.projectile.explosionRadius * 0.7f), Map);
            }
        }
